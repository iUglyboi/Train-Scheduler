$(document).ready(function() {
  let getInnerText = function(element) {
    let html = element.find('code')[0].outerHTML;
    let proxyItem = document.createElement('div');
    proxyItem.innerHTML = html;
    return proxyItem.textContent;
  };

  let createButton = function(className) {
    let btn = document.createElement('div');
    btn.className = className;
    return btn;
  };

  let startScreenView = function() {
    let content = this.dataset !== undefined ? this.dataset.clipboardText : this.getAttribute('data-clipboard-text');
    window.sessionStorage.setItem('content', content);
    let title = $(this).parent().parent().parent().parent().find('h2')
      .clone(true)
      .find('a')
      .remove()
      .end()
    ...
    $('.fs-modal .modal-body').delegate('.reqres a', 'click', function(event) {
      ...
      let reqresNo = $(this).parent().parent().attr('class').substr(7);
      $('.left-wrapper .reqres.' + reqresNo + ' a').eq($(this).index()).click();
    });
  };

  let startRowView = function() {
    let content = this.dataset !== undefined ? this.dataset.contentText : this.getAttribute('data-content-text');
    window.sessionStorage.setItem('content', content);
    let win = window.open(window.location.protocol + '//' + window.location.host + '/products-and-docs/raw-view/', '_blank');
    win.focus();
  };

  let makeCopy = function() {
  ...
  $('.reqres').each(function(index) {
    ...
    let screenbtn1 = createButton('screen-btn');
    ...
    let rawbtn1 = createButton('raw-btn');
    ...
    let btn1 = createButton('copy-btn');
    ...
    let screenbtn2 = createButton('screen-btn');
    ...
    let rawbtn2 = createButton('raw-btn');
    ...
    let btn2 = createButton('copy-btn');
